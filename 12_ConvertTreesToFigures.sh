...
then
	...
	echo "Skipping $inputTree - $cladeTreeFile already exists (pass -U/--updateBig to force)." >&2
fi
...
then
	echo "Skipping $inputTree - $outputFile already exists (pass -u/--update to force; only has an effect here if --iteration differs from --baseIteration - if this is the master tree at its own base iteration, this file is only ever regenerated by the first block above, gated by -U/--updateBig)." >&2
fi
...
do
	...
	then
		...
		echo "Skipping $inputTree - $outputFile already exists (pass -u/--update to force)." >&2
	fi
done
