...
if [[ "$allSeqs" == "--allSeqs" ]]
then
	# If we run against the wall, just restart the main task. sacct lets
	# us check *why* the job ended, not just whether it succeeded, so
	# the check above can restrict the actual restart to a genuine
	# TIMEOUT instead of any non-success (including a manual scancel).
	# Without sacct (e.g. PBS), fall back to the old afternotok behavior
	# - PBS has no equally simple way to ask "was this killed for
	# walltime specifically."
	...
	"$DIR/Scheduler-Sub.sh" -v "DIR=$DIR, gene=$gene, iteration=$iteration, aligner=$aligner, numRoundsLeft=$numRoundsLeft, bigNumRoundsLeft=$bigNumRoundsLeft, shuffleSeqs=$shuffleSeqs, allSeqs=$allSeqs, suffix=$suffix, extension=$extension, previousAligner=$previousAligner, trimAl=$trimAl, bigTreeIteration=$bigTreeIteration$holdJobsExport" -W "depend=$restartDepend" \
